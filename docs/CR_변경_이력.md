...
- **Aimbase 실측**: Aimbase에 실행 중 워크플로우 run을 취소하는 REST API **미존재** — [WorkflowController](../../../bp-platform/aimbase/backend/platform-core/src/main/java/com/platform/api/WorkflowController.java)는 run/approve/조회만, [WorkflowEngine](../../../bp-platform/aimbase/backend/platform-core/src/main/java/com/platform/workflow/WorkflowEngine.java)에 cancel 메서드 없음. 단 [SubagentController](../../../bp-platform/aimbase/backend/platform-core/src/main/java/com/platform/api/SubagentController.java)에 `POST /api/v1/agents/{runId}/cancel` 존재 → 향후 Aimbase가 WF 취소 API를 만들 때 참고 패턴. Aimbase 쪽 작업은 별도 진행.
- **변경 사항**:
...
- **상태**: 설계 캐스케이드 진행중 (2026-06-14). 구현 대기.
