...
| CR-034 | 2026-06-03 | PIEE 입찰서류 정본 감지·링크 노출 — 본문(noticedesc)에 piee.eb.mil 제출 지정이 있는 공고를 `piee_available`로 표식(applyDescriptionBody 검출), 관리자 상세에 "입찰서류(PIEE)" 섹션으로 PIEE oppMgmtLink(solNo 기반) 링크 노출. 자동 다운로드 불가(세션 웹앱)이므로 수동 안내. CR-025(SAM 자체호스팅 자동 다운로드)와 별개 성격 | BE (entity/Opportunity piee_available 필드·applyDescriptionBody 검출, dto/OpportunityAdminDto pieeAvailable/pieeUrl, 마이그레이션 V30) + FE (admin-console OpportunityMetaPanel PIEE 블록·OpportunityAdminDetailPage) | 소규모 | 구현·BE 컴파일·FE 타입체크 통과 (2026-06-03) |
