...
### TextExtractionStatus (파일 텍스트 추출 상태) — CR-013 신규

| 값 | 설명 |
|----|------|
| PENDING | 추출 대기 |
| SUCCESS | 텍스트 추출 성공 |
| FAILED | 추출 실패 (브로슈어형 PDF 등 — 수동 입력 fallback) |
...
### SlotDefinition (표준 슬롯 정의) — **CR-013 신규**

| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| id | BINARY(16) | PK | auto | UUID |
| slot_code | VARCHAR(40) | Y | | 슬롯 코드 (UNIQUE). 예: PRIOR_EXPERIENCE |
| label_ko | VARCHAR(100) | Y | | 한글 라벨. 예: 과거 수행 경험 |
| display_order | INT | Y | | 정렬 순서 |
| is_other | BOOLEAN | Y | false | "기타" 슬롯 여부 (라벨 직접 입력 허용) |
| created_at / updated_at | DATETIME(6) | Y | now() | |

**인덱스**: `uniq_slot_definition_code` — slot_code (UNIQUE)

**설계 원칙**:
- 7슬롯 + 기타 1슬롯을 **시드 데이터(Flyway INSERT)로 정의** — 코드 하드코딩 금지. 향후 슬롯 추가·공고유형별 세트 확장 대비
- 시드 8행: BIZ_QUALIFICATION/PRIOR_EXPERIENCE/KEY_PERSONNEL/STAFFING_PLAN/EQUIPMENT_PLAN/PAST_PERFORMANCE/PRICE/OTHER
...
| pws_text | LONGTEXT | N | | (선택) PWS 공고문 추출 텍스트 (A경로용) |
...
| extracted_text | LONGTEXT | N | | 등록 시 1회 추출한 텍스트 (패턴추출·검색용) |
| extraction_status | TextExtractionStatus | Y | PENDING | 텍스트 추출 상태 |
| created_at / updated_at | DATETIME(6) | Y | now() | |
...
### SlotAssignment (슬롯 배치) — **CR-013 신규**

| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| id | BINARY(16) | PK | auto | UUID |
| rfp_sample_id | BINARY(16) | FK | | → RfpSample.id (ON DELETE CASCADE) |
| slot_definition_id | BINARY(16) | FK | | → SlotDefinition.id |
| sample_file_id | BINARY(16) | FK | N | → RfpSampleFile.id (파일 단위 배치 시) |
| other_label | VARCHAR(100) | N | | slot=OTHER일 때 라벨 |
| section_text | LONGTEXT | N | | 파일 일부 섹션만 배치 시 / 추출 실패 수동 입력 fallback |
| confirmed | BOOLEAN | Y | false | 자동추정 후 관리자 확인 여부 |
| auto_estimated | BOOLEAN | Y | false | 시스템 자동추정으로 생성됐는지 |
| created_at / updated_at | DATETIME(6) | Y | now() | |

**인덱스**: `idx_slot_asg_sample` — rfp_sample_id / `idx_slot_asg_slot` — slot_definition_id

---

### PatternGuide (슬롯별 패턴 가이드) — **CR-013 신규**

| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| id | BINARY(16) | PK | auto | UUID |
| slot_definition_id | BINARY(16) | FK | | → SlotDefinition.id |
| industry_type | IndustryType | N | | 사업유형 (1차엔 NULL=공통 허용) |
| status | ExtractionStatus | Y | PENDING | 추출 상태 |
| source | GuideSource | Y | AI_EXTRACTED | 출처 (AI 자동갱신 vs 사람편집 보호) |
| guide_json | JSON | N | | 구조화 가이드 (8블록 골격/체크리스트/금기 — LLM 입력용) |
...
**인덱스**:
- `uniq_guide_slot_industry` — (slot_definition_id, industry_type) (UNIQUE — 슬롯·유형당 1가이드, 부분 재추출은 같은 행 갱신)
- `idx_pattern_guide_status` — status

**설계 원칙**:
- 추출 단위 = 슬롯(BIZ-016). 같은 슬롯에 모인 여러 건 비교 → 공통 패턴
- 출처 보호(BIZ-017): source=HUMAN_*인 가이드는 AI 자동추출이 덮어쓰지 않음
- guide_json은 `docs/rfp_pattern_guide_draft_prior_experience.md`의 8블록/체크리스트/금기 구조를 계약으로 사용 (save_pattern_guide MCP 입력 스키마)
...
erDiagram
    ...
    RfpSample ||--o{ RfpSampleFile : "원본파일 (CR-013)"
    RfpSample ||--o{ SlotAssignment : "슬롯배치 (CR-013)"
    SlotDefinition ||--o{ SlotAssignment : "슬롯정의 (CR-013)"
    SlotDefinition ||--o{ PatternGuide : "슬롯별 가이드 (CR-013)"
    RfpSampleFile }o--o| SlotAssignment : "파일배치 (CR-013)"
```
